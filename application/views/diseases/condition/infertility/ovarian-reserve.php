<main>
	<!-- Breadcumb -->
	<div class="breadcumb-wrapper ">
		<div class="parallax" data-parallax-image="<?= base_url("assets/") ?>img/breadcurmb/breadcurmb-1-1.jpg"></div>
		<div class="container z-index-common">
			<div class="breadcumb-content">
				<h1 class="breadcumb-title">Decrease Ovarian Reserve</h1>
				<div class="breadcumb-menu-wrap">
					<i class="far fa-home-lg"></i>
					<ul class="breadcumb-menu">
						<li><a href="<?= base_url() ?>">Home</a></li>
						<li class="active">Decrease Ovarian Reserve</li>
					</ul>
				</div>
			</div>
		</div>
	</div>
	<!--
    Service Area
    -->
	<section class="vs-service-wrapper space-top">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<div class="service-content mb-30">
						<div class="vs-surface wow" data-wow-delay="0.3s">
							<picture>
								<source media="(min-width: 800px)" srcset="https://placehold.co/1200x600" type="image/webp">
								<source media="(min-width: 800px)" srcset="https://placehold.co/1200x600" type="image/jpg">
								<source srcset="https://placehold.co/800x800" type="image/webp">
								<img src="https://placehold.co/800x800" alt="Service Image" class="w-100">
							</picture>
						</div>
						<div class="row justify-content-center">
							<div class="col-xl-11 col-xxl-10 ">
								<div class="service-bar">
									<div class="row justify-content-between align-items-center gy-4 text-center text-lg-start">
										<div class="col-sm-6 col-lg-auto">
											<span class="fs-xs">Condition Name</span>
											<h2 class="h5 mb-0">Decrease Ovarian Reserve</h2>
										</div>

										<div class="col-sm-6 col-lg-auto">
											<a href="#appointment" class="vs-btn style2">Appointment<i class="fal fa-calendar-alt"></i></a>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="vs-detail-wrapper space-md-bottom">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<div class="">
						<h2 class="h1">Unlocking Hope: StemRx Hospital's Breakthrough Approach to Boosting Ovarian Reserve!</h2>
						<p class="fs-md mb-4 pb-2">
							Decreased ovarian reserve is a condition where a woman's ovaries contain fewer eggs than normal, or the eggs that remain are of lower quality. This can lead to difficulties in conceiving naturally and an increased risk of infertility. It typically occurs as women age, but can also result from certain medical treatments, such as chemotherapy, or underlying health conditions. Symptoms may include irregular menstrual cycles or difficulty getting pregnant. Early detection through hormone testing and ovarian reserve assessments can help women understand their fertility potential and explore options such as assisted reproductive technologies to achieve pregnancy.
						</p>
					</div>
				</div>
				<div class="col-12">
					<div class="">
						<h2>Symptoms</h2>
						<p>
							Here are the key symptoms of Decrease Ovarian Reserve:
						</p>
						<ul class="row list-unstyled">
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Irregular menstrual cycles.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Difficulty conceiving.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Increased FSH (Follicle Stimulating Hormone) levels.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Decreased AMH (Anti-Müllerian Hormone) levels.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Poor response to ovarian stimulation during fertility treatments.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Premature menopause symptoms (like hot flashes and mood swings) at a younger age.</h5>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12 p-md-3 p-0">
								<div class="">
									<h5 class="m-0">Diminished ovarian reserve on ultrasound (fewer follicles visible).</h5>
								</div>
							</li>
						</ul>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="vs-detail-wrapper py-5 bg-light">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<div class="">
						<h2>Common Fears and Concerns</h2>
						<ul class="row list-unstyled">
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Fertility Issues:</h6>
									<p>Reduced ovarian reserve can lead to difficulties in conceiving naturally, increasing the fear of infertility.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Diminished Egg Quality:</h6>
									<p>With fewer eggs available, there's a concern about the quality of the remaining eggs, potentially leading to higher chances of miscarriage or chromosomal abnormalities in embryos.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Impact on Reproductive Timeline:</h6>
									<p>Women may worry about their ability to conceive within their desired timeframe, especially if they haven't yet started or completed their family planning.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Emotional Stress:</h6>
									<p>Dealing with the uncertainty of fertility and facing the possibility of not being able to conceive can cause significant emotional distress and anxiety.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Financial Burden:</h6>
									<p>Fertility treatments such as IVF can be expensive, and the need for multiple cycles due to decreased ovarian reserve can add to the financial strain.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Decision Making:</h6>
									<p>Women may feel pressured to make decisions about starting a family sooner than planned or to explore alternative options such as egg freezing.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Relationship Strain: </h6>
									<p> Difficulty conceiving and the emotional toll it takes can strain relationships, leading to conflicts or feelings of inadequacy.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Social Stigma:</h6>
									<p>There might be feelings of shame or societal pressure associated with fertility issues, leading to reluctance in seeking support or discussing the topic openly.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Health Concerns:</h6>
									<p>Some may worry that decreased ovarian reserve could be a sign of underlying health issues, prompting concerns about overall well-being and future health complications.</p>
								</div>
							</li>
							<li class="col-lg-4 col-md-6 col-12">
								<div class="">
									<h6 class="m-0">Future Planning:</h6>
									<p>Planning for a future with uncertain fertility can be challenging, impacting decisions related to career, education, and lifestyle choices.</p>
								</div>
							</li>
						</ul>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="vs-video-wrapper py-5 d-none">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-xl-10 col-12">
					<div class="w-100">
						<iframe class="ratio ratio-16x9" src="https://www.youtube.com/embed/1GeNmwf-qpY?si=7V36-dC9SW6GUmm1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="vs-detail-wrapper space appointment-wrapper">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<div class="">
						<h3>Why StemRx:</h3>
						<p>StemRx is the only healthcare services organization that offers a complete end to end healthcare package under one single roof right from the point where the patient walks in for a consultation, the evaluation, the medical assessments, the therapies, the medication, long term post discharge strategies and much more.</p>
						<p>One look at the patient feedback library will indicate that at StemRx we are committed to improvement of medical condition of the patient by meeting the defined timeline-based milestones, as well as ensuring that there is patient delight when the patient departs from our facility. </p>
					</div>
				</div>
				<div class="col-lg-4 col-md-6 col-12">
					<div class="">
						<h4><b class="text-title">EXPERTISE :</b></h4>
						<div class="row gx-40">
							<div class="col-xl-12">
								<ul>
									<li>More than 250 plus years of cumulative experience of the team coming from various specialties</li>
									<li>Handling 3 locations and global citizens</li>
									<li>Quick identification of required personalized attention to be given to each patient</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
				<div class="col-lg-4 col-md-6 col-12">
					<div class="">
						<h4><b class="text-title">EXCELLENCE :</b></h4>
						<div class="row gx-40">
							<div class="col-xl-12">
								<ul>
									<li>Our continued benchmarks of improving upon prior set quality and delivery objectives</li>
									<li>Well documented and tested treatment protocols with an eye to adhere to global standards</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
				<div class="col-lg-4 col-md-6 col-12">
					<h4><b class="text-title">EQUITY :</b></h4>
					<div class="row gx-40">
						<div class="col-xl-12">
							<ul>
								<li>A complete holistic, ethical, and natural solution using regenerative medicine.</li>
								<li>Complete E2E customized & personalized treatment protocols</li>
								<li>Only organization to take care of patient from first contact to last mile post discharge documentation</li>
								<li>Strict quality control measures and GMP certified labs in place</li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>


	<section class="vs-specialities-wrapper space-md-bottom">
		<div class="container">
			<div class="row text-center justify-content-center mb-30">
				<div class="col-md-10 col-lg-8 wow fadeInUp" data-wow-delay="0.3s">
					<h2 class="h1">Our Treatments at StemRx</h2>
					<!-- <p>Proactively revolutionize granular customer service after pandemic internal or "organic" sources proactive human capital rather.</p> -->
				</div>
			</div>
		</div>
		<div class="row justify-content-center">
			<div class="col-lg-9 col-md-11 col-12">
				<div class="row g-4 justify-content-center">
					<div class="col-lg-auto col-md-6 col-12">
						<a href="<?= base_url("therapeutics/fmt") ?>" class="text-center">
							<div class="card bg-white sp-item mb-2">
								<div class="card-body">
									<p class="m-0">Intestinal Microbiota Transfer (IMT)</p>
								</div>
							</div>
						</a>
					</div>
					<div class="col-lg-auto col-md-6 col-12">
						<a href="<?= base_url("therapeutics/detoxification-therapy") ?>" class="text-center">
							<div class="card bg-white sp-item mb-2">
								<div class="card-body">
									<p class="m-0">Detoxification Therapy</p>
								</div>
							</div>
						</a>
					</div>
					<div class="col-lg-auto col-md-6 col-12">
						<a href="javascript:void(0)" class="text-center">
							<div class="card bg-white sp-item mb-2">
								<div class="card-body">
									<p class="m-0">Stem Cell Therapy</p>
								</div>
							</div>
						</a>
					</div>
					<div class="col-lg-auto col-md-6 col-12">
						<a href="javascript:void(0)" class="text-center">
							<div class="card bg-white sp-item mb-2">
								<div class="card-body">
									<p class="m-0">Quantum Energy Medicine</p>
								</div>
							</div>
						</a>
					</div>
				</div>
			</div>
		</div>
		</div>
	</section>

	<section class="vs-team-wrapper space-md-bottom">
		<div class="container">
			<div class="team-content text-center mb-30">
				<!-- <span class="sec-subtitle text-theme h3 mb-2 mb-sm-0">StemRx Medical Care!</span> -->
				<h2 class="h1 mb-3">Our Consultant for Diseses</h2>
			</div>
			<div class="row justify-content-center">
				<div class="col-md-6 col-lg-4 col-xl-3 mb-30 wow fadeInUp" data-wow-delay="0.3s">
					<div class="team-card">
						<div class="team-head"><img src="<?= base_url() ?>assets/img/team/drpvmahajan.jpg" alt="Team Area" class="w-100">
							<div class="team-card-links"><a class="team-links-toggler" href="<?= base_url('doctor/') ?>dr-pradeep-mahajan"><i class="fas fa-eye"></i></a>
								<!-- <div class="team-social"><a href="#"><i class="fab fa-facebook-f"></i></a> <a href="#"><i class="fab fa-twitter"></i></a> <a href="#"><i class="fas fa-basketball-ball"></i></a></div> -->
							</div>
						</div>
						<div class="team-body p-3">
							<h3 class="h4 mb-0"><a href="<?= base_url('doctor/') ?>dr-pradeep-mahajan" class="text-reset">Dr. Pradeep Mahajan</a></h3>
						</div>
					</div>
				</div>
				<div class="col-md-6 col-lg-4 col-xl-3 mb-30 wow fadeInUp" data-wow-delay="0.3s">
					<div class="team-card">
						<div class="team-head"><img src="<?= base_url() ?>assets/img/team/dr.pallavi.jpg" alt="Team Area" class="w-100">
							<div class="team-card-links"><a class="team-links-toggler" href="<?= base_url('doctor/') ?>dr-pallavi-lele"><i class="fas fa-eye"></i></a>
								<!-- <div class="team-social"><a href="#"><i class="fab fa-facebook-f"></i></a> <a href="#"><i class="fab fa-twitter"></i></a> <a href="#"><i class="fas fa-basketball-ball"></i></a></div> -->
							</div>
						</div>
						<div class="team-body p-3">
							<h3 class="h4 mb-0"><a href="<?= base_url('doctor/') ?>dr-pallavi-lele" class="text-reset">Dr. Pallavi Lale</a></h3>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="vs-skill-wrapper space-md-bottom d-none">
		<div class="container">
			<div class="skill-wrap1 bg-white">
				<div class="row justify-content-center justify-content-lg-between">
					<div class="col-md-6 col-lg-auto mb-30">
						<div class="d-xl-flex text-center text-xl-start skill-box"><span class="ripple-icon hover-style2 align-self-start mb-20 mb-xl-0 mr-20"><i class="flaticon-discuss"></i></span>
							<div class="media-body">
								<h2 class="mt-n2 mb-0 text-theme">30+</h2>
								<p class="text-title fs-md fw-medium mt-1 mt-xl-0 mb-2 mb-xl-2">Years Of Experience</p>
								<p class="fs-xs mb-0">Incubate extensive scenarios without top-line quality vectors. Authoritatively engage</p>
							</div>
						</div>
					</div>
					<div class="col-md-6 col-lg-auto mb-30">
						<div class="d-xl-flex text-center text-xl-start skill-box"><span class="ripple-icon hover-style2 align-self-start mb-20 mb-xl-0 mr-20"><i class="flaticon-medical-equipment"></i></span>
							<div class="media-body">
								<h2 class="mt-n2 mb-0 text-theme">100+</h2>
								<p class="text-title fs-md fw-medium mt-1 mt-xl-0 mb-2 mb-xl-2">Experienced Doctor's</p>
								<p class="fs-xs mb-0">Incubate extensive scenarios without top-line quality vectors. Authoritatively engage</p>
							</div>
						</div>
					</div>
					<div class="col-md-6 col-lg-auto mb-30">
						<div class="d-xl-flex text-center text-xl-start skill-box"><span class="ripple-icon hover-style2 align-self-start mb-20 mb-xl-0 mr-20"><i class="flaticon-healthcare"></i></span>
							<div class="media-body">
								<h2 class="mt-n2 mb-0 text-theme">200+</h2>
								<p class="text-title fs-md fw-medium mt-1 mt-xl-0 mb-2 mb-xl-2">Happy Patients</p>
								<p class="fs-xs mb-0">Incubate extensive scenarios without top-line quality vectors. Authoritatively engage</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="testimonial-wrapper space-bottom d-none">
		<div class="container">
			<div class="row text-center justify-content-center">
				<div class="col-md-10 col-lg-8 col-xl-6 wow fadeInUp" data-wow-delay="0.3s">
					<div class="section-title">
						<!-- <span class="h3 text-theme sec-subtitle">Testimonial</span> -->
						<h2 class="h1">Our Patients Responses</h2>
						<!-- <p>Proactively revolutionize granular customer service after pandemic internal or "organic" sources proactive human capital rather.</p> -->
					</div>
				</div>
			</div>
			<div class="row justify-content-center">
				<div class="col-xl-10">
					<div class="row vs-carousel wow fadeInUp" data-wow-delay="0.3s" data-slide-show="3">
						<div class="col-xl-6">
							<a href="https://www.youtube.com/watch?v=_m5XOHuJAxI&feature=youtu.be" class="ratio-16x9" target="_blank">
								<div class="youtube-testimonial">
									<img src="<?= base_url() ?>assets/img/testimonial/thumbnail-1.jpg" alt="">
								</div>
							</a>
						</div>
						<div class="col-xl-6">
							<a href="https://www.youtube.com/watch?v=_m5XOHuJAxI&feature=youtu.be" class="ratio-16x9" target="_blank">
								<div class="youtube-testimonial">
									<img src="<?= base_url() ?>assets/img/testimonial/thumbnail-1.jpg" alt="">
								</div>
							</a>
						</div>
						<div class="col-xl-6">
							<a href="https://www.youtube.com/watch?v=_m5XOHuJAxI&feature=youtu.be" class="ratio-16x9" target="_blank">
								<div class="youtube-testimonial">
									<img src="<?= base_url() ?>assets/img/testimonial/thumbnail-1.jpg" alt="">
								</div>
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>

	<section class="appointment-wrapper space-md-bottom" id="appointment">
		<div class="container">
			<div class="row">
				<div class="col-xl-6 mb-30">
					<div class="about-content">
						<!-- <span class="sec-subtitle text-theme h3 mb-2 mb-sm-0">StemRx Medical Care!</span> -->
						<h2 class="h1 mb-3">We're Here For You</h2>
						<div class="row">
							<div class="col-12">
								<div class="vs-accordion accordion accordion-style2" id="vs-faq-accordion">
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse1" aria-expanded="false" aria-controls="collapse1">Regenerative Potential</button></h2>
										<div id="collapse1" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion" style="">
											<div class="accordion-body">
												<p>
													StemRx harnesses the regenerative power of stem cells to repair and regenerate damaged muscle tissue. This approach goes beyond symptom management to address the underlying causes of muscular degeneration.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded="false" aria-controls="collapse2">Specialized Expertise</button></h2>
										<div id="collapse2" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													StemRx Hospital boasts a team of leading fertility specialists dedicated to addressing decreased ovarian reserve with precision and care.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse3" aria-expanded="false" aria-controls="collapse3">Cutting-Edge Technology</button></h2>
										<div id="collapse3" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													Utilizing the latest advancements in assisted reproductive technology (ART), StemRx offers state-of-the-art treatments tailored to each patient's specific needs.
												</p>
											</div>
										</div>
									</div>

									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse5" aria-expanded="false" aria-controls="collapse5">Innovative Stem Cell Therapy</button></h2>
										<div id="collapse5" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													StemRx Hospital stands out for its pioneering approach, integrating stem cell therapy with traditional fertility treatments to enhance ovarian function and improve outcomes.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse4" aria-expanded="false" aria-controls="collapse4">Comprehensive Care</button></h2>
										<div id="collapse4" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													Beyond treatment, StemRx provides comprehensive support services, including counseling and holistic wellness programs, ensuring patients receive personalized care every step of the way.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse6" aria-expanded="false" aria-controls="collapse6">Research and Development</button></h2>
										<div id="collapse6" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													StemRx is committed to advancing the field of reproductive medicine through ongoing research and development, continually refining techniques and protocols to achieve optimal results for patients with decreased ovarian reserve.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse7" aria-expanded="false" aria-controls="collapse7">Success Rates</button></h2>
										<div id="collapse7" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													With a track record of success and a focus on evidence-based practice, StemRx Hospital consistently delivers high success rates, giving hope to those struggling with decreased ovarian reserve.
												</p>
											</div>
										</div>
									</div>
									<div class="accordion-item py-3">
										<h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse8" aria-expanded="false" aria-controls="collapse8">Patient-Centred Approach</button></h2>
										<div id="collapse8" class="accordion-collapse collapse" data-bs-parent="#vs-faq-accordion">
											<div class="accordion-body">
												<p>
													StemRx prioritizes patient comfort and satisfaction, offering a compassionate and supportive environment where individuals feel empowered and informed throughout their fertility journey.
												</p>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="d-none">
							<ul class="offer-counter counter-list list-unstyled" data-offer-date="01/01/2024">
								<li>
									<div class="day count-number">00</div><span class="count-name">Days</span>
								</li>
								<li>
									<div class="hour count-number">00</div><span class="count-name">Hours</span>
								</li>
								<li>
									<div class="minute count-number">00</div><span class="count-name">Minutes</span>
								</li>
								<li>
									<div class="seconds count-number">00</div><span class="count-name">Seconds</span>
								</li>
							</ul>
							<div class="vs-code-box mt-20">Use Code: <span class="text-theme">Native</span></div>
						</div>
					</div>
				</div>
				<div class="col-xl-6 mb-30 pt-30 pt-xl-0">
					<div class=" bg-white wow fadeInUp">
						<?php
						$data = [
							"action" => [
								"method" => "POST",
								"action" => base_url()
							],
							"title" => [
								'bg-image' => "",
								'heading' => "Book An Appointment",
								'desc' => "Please Call Us To Ensure",
							],
							"props" => [
								'class' => "shadow-none mb-0",
								'bg-image' => "",
								'bg-color' => "#f3f6f7",
							],
							"fields" => [
								"condition" => "disabled",
								"doctor" => "enabled",
							]
						];
						$this->load->view('common/forms/appointment_form', $data); ?>
					</div>
				</div>
			</div>
		</div>
	</section>
</main>
